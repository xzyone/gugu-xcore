...
// 实现方(gugu-agent)可以根据 email 查 per-user rate limiter,把 rawConn 包一层 throttling conn 返回。
//
// 钩子返回 nil 或返回 rawConn 本身 → vision 走原路径(零拷贝、不限速)。
// 返回新 conn → vision 后续 raw IO 全部经过该 conn,逃不掉 user-space 节流。
//
// 这个钩子只对启用 xtls-rprx-vision 的连接生效;其他协议的限速仍走 gugu-agent dispatcher 标准 RateWriter 路径。
